...
void LCD_Bitmap_Mono(uint16_t x, uint16_t y, const char *bitmap, uint32_t color24_set, uint32_t color24_unset)
{
	...
	for (uint16_t i = 0; i < h; i++)
	{
		for (uint16_t j = 0; j < w; j++)
		{
			if (bit_pos % 8 == 0) byte = (*(const unsigned char *)(bitmap++));
...
const uint8_t LCD_Blue_Init[] = // Initialization commands for 7735B screens
...
const uint8_t LCD_Red_Init_1[] = // Init for 7735R, part 1 (red or green tab)
...
const uint8_t LCD_Red_Init_Green_2[] = // Init for 7735R, part 2 (green tab only)
...
const uint8_t LCD_Red_Init_Red_2[] =  // Init for 7735R, part 2 (red tab only)
{	2,                 	//  2 commands in list:
	CASET, 4,  	//  1: Column addr set, 4 args, no delay:
	0x00, 0x00,     //     XSTART = 0
	0x00, 0x7F,     //     XEND = 127
...
const uint8_t LCD_Red_Init_Green_144_2[] = // Init for 7735R, part 2 (green 1.44 tab)
{	2,                    	//  2 commands in list:
	CASET, 4,  		//  1: Column addr set, 4 args, no delay:
	0x00, 0x00,         //     XSTART = 0
	0x00, 0x7F,         //     XEND = 127
...
const uint8_t LCD_Red_Init_3[] = // Init for 7735R, part 3 (red or green tab)
...
void LCD_Command_List(const uint8_t *addr)
{
	...
	cmd_count = (*(const unsigned char *)(addr++));   // Number of commands to follow
	for(uint8_t cmd_pos = 0; cmd_pos < cmd_count; cmd_pos++)
	{
		LCD_Command((*(const unsigned char *)(addr++))); 	// Read, send command
		arg_count  = (*(const unsigned char *)(addr++));    		// Number of args to follow
		...
		{ // For each argument...
			LCD_Data_8((*(const unsigned char *)(addr++)));  		// Read, send argument
		}
